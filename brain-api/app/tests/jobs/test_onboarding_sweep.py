...
async def test_sources_run_in_authority_order() -> None:
    ...
    result, _, synced = await _run(sources, {})
...
async def test_all_sources_failing_marks_sweep_failed() -> None:
    ...
    result, repo, _ = await _run(sources, {"src_notion": RuntimeError("down")})
...
async def test_resume_skips_sources_already_completed() -> None:
    ...
    result, _, synced = await _run(sources, {}, sweep=sweep)
...
async def test_missing_sweep_row_is_a_noop() -> None:
    result, _, synced = await _run([], {}, sweep=None)
    # sweep=None short-circuits before set_sweep_status
