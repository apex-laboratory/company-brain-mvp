...
Mocks at the raw-call boundary (``_groq_call``) so no SDK or network is touched.
"""
...
from app.pipeline.llm.clients import LLMParseError, _parse_json, groq_json
...
async def test_groq_json_returns_parsed_and_usage() -> None:
    raw = AsyncMock(return_value=('{"relevant": true}', 100, 20))
    with patch.object(clients, "_groq_call", raw):
        parsed, usage = await groq_json("sys", "user", stage="relevance_gate")
...
async def test_non_json_reprompts_once_and_sums_usage() -> None:
    ...
    with patch.object(clients, "_groq_call", raw):
        parsed, usage = await groq_json("sys", "user", stage="t")
    assert parsed == {"ok": 1}
...
async def test_non_json_twice_raises_parse_error() -> None:
    ...
    with patch.object(clients, "_groq_call", raw), pytest.raises(LLMParseError):
        await groq_json("sys", "user", stage="t")
    assert raw.await_count == 2
...
def test_missing_key_raises_at_first_use(monkeypatch) -> None:
    ...
    monkeypatch.setattr(clients, "_groq_client", None)
    # settings is a frozen pydantic model — swap the module reference instead.
    monkeypatch.setattr(clients, "settings", SimpleNamespace(groq_api_key=""))
