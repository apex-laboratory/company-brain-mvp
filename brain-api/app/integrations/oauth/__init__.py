"""Third-party OAuth provider integrations (login SSO).

Each sub-module owns the HTTP exchange (code → access_token → profile) for one
login provider. All calls use ``httpx.AsyncClient`` with a 10-second timeout so a
hung provider never stalls the request indefinitely.

Distinct from ``app.integrations`` (the read-only ``SourceIntegration`` protocol
for connected data sources) — this package is purely "Sign in with X".
"""
