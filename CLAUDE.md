...
| — LLM provider factory | `app/pipeline/llm/providers.py` picks Gemini/Anthropic/OpenRouter behind `clients.llm_json`/`llm_stream`, selected by the single `LLM_PROVIDER` env var — the whole app (5 pipeline stages + `app/modules/brain/synthesizer.py`) runs on one provider at a time, swappable with no code changes. Default is `gemini` (`GEMINI_API_KEY`); `openrouter` (`OPENROUTER_API_KEY`, `:free`-suffixed models cost $0) is for testing/demos without paid keys. `ANTHROPIC_API_KEY` is unset/placeholder in this deployment, so `LLM_PROVIDER=anthropic` 401s at runtime until a key is set. | ✅ |
...
| — Brainite plugin (the client) | `plugin/` — Go binary + Claude Code hooks that turn a coding session into `POST /runs` envelopes and inject matched skills back at `UserPromptSubmit`. **Hook payload field names are pinned by real captured fixtures in `plugin/testdata/golden/`, which correct `docs/PLUGIN_BUILD.md` in six places — the fixtures win.** The load-bearing one: **a failed tool call emits `PreToolUse` and no `PostToolUse`**, so `PreToolUse` must be hooked (the doc's `hooks.json` omits it) and a step opened-but-never-closed is the *only* evidence of an error. Drop it and every run looks clean and the gate's error-free-tail check is defeated. `brain-api/app/tests/runs/test_plugin_contract.py` validates the client's real output against `RunIngestRequest` so the two languages can't drift. | ✅ milestones 1–4 |
| 7 Agent-run ingestion (self-improving loop) | **Ingest half built** (branch `feat/agent-run-ingestion`): `POST /runs` (scope `runs:write`, **write-only** — there is deliberately no `GET /runs`; a trace holds file contents and shell output, so the product's answer to "what did the agent do" is the reviewed procedure, never the evidence), `agent_runs` (migration 0025, RLS), two-pass redaction (`app/modules/runs/redaction.py` — fails closed: an unredactable trace is stored as a run with no body), and the `gate_run` job (Feature 30 — deterministic, **no LLM**: outcome + override + clean tail + step bounds + digest dedupe, then cosine-0.85 task clustering; distillation fires per *cluster* at `run_min_runs_per_cluster`, or immediately on `humanConfirmed`). **Still spec-only:** trajectory compression (F31), procedure extractor (F32), review card (F33), reinforcement (F34), backfill (F29a) — the seam is marked in `gate_run.py`. Client shim: `docs/AGENT_HOOK_SHIM.md`. | 🔵 in progress |
| — ⚠️ MCP public ingress | **Open blocker, infrastructure.** `query_brain` serves on compose-internal port 8001; nothing routes `https://mcp.brainites.com/mcp` to it. Anthropic's MCP proxy connects from *their* side over public HTTPS, so no agent can be grounded until this lands — and the plugin cannot be submitted to either marketplace. Two fixes are **ours, not the ingress owner's**, and must land with it: neither surface reads `X-Forwarded-For`, so `enforce_api_key_probe_limit` (10/min, keyed on the socket peer) collapses into **one global bucket the day a proxy appears** — a hard outage at 10 req/min worldwide; and the MCP app exposes only `/mcp`, with no health route for a load balancer to check. Full handoff: `docs/MCP_INGRESS.md`. | 🔴 blocked |
| — Agent builder | User-built agents that reach **their own** tools at runtime over MCP, grounded in the Brain. Runtime is **Anthropic Managed Agents** — the one part of the app that can't ride the `LLM_PROVIDER` factory. Plan: `docs/agent-builder-plan.md`. **Brainite stores pointers — agents, vaults, sessions, schedules — never the content that flows through them**, and that wall is why `agent_credentials` has no token column and `agent_sessions` has no transcript column (migration 0028 says so; §5.5's schema test will enforce it). Phase 0 (streamable-HTTP MCP + bearer auth + the `agent_origin` query-text guard) ✅ and phase 1 (six tables, agent CRUD, connectors, `anthropic_client.py` — the only file that imports the Managed Agents SDK) ✅ are on `main`. Phase 2 (catalog, per-connector OAuth, vaults) is on `feat/agent-builder-credentials`; phase 4 (sessions, dual vaults, SSE proxy, webhook) is on `feat/agent-builder-sessions`. **Connectors here are data, not code** — a provider is a row in `agent_connectors.yaml` plus env-var client credentials; do *not* copy the compile-time `SourceIntegration` pattern from `app/integrations/`. Remaining: phase 6 schedules, phase 7 wall-enforcement tests. | 🔵 in progress |
| — Agent sessions vs. the plan | Three places the SDK (0.109.1) contradicts `agent-builder-plan.md`, all load-bearing. **(1) §5.6's session budget does not exist** — no `budget` on `sessions.create` or `deployments.create`, no cost field to read one back; `agent_definitions.budget_cents` is recorded and *not enforced*, and the only real brake is `MAX_LIVE_SESSIONS_PER_USER` (5 concurrent). Don't build a spend UI on that column. **(2) `sessions.create` requires an `environment_id`** the plan never mentions — pin `ANTHROPIC_ENVIRONMENT_ID` in any multi-process deployment or the find-or-create fallback races and leaves two same-named environments no session row can tell apart. **(3) `vault_ids` is create-only** (`sessions.update` rejects it), so §5.4's pair attaches at create or the session runs unauthorized for life. Also: `ground_in_brain` had reached *nothing* since 0028 — the flag was stored and never put the Brain's MCP server on the agent — so a grounded agent now spends one of Anthropic's 20 connector slots and the user's ceiling is 19. | ✅ |
| — Agent session streaming | `GET /agent-sessions/{id}/stream` is a byte-faithful SSE pass-through: **no accumulator, ever** (§5.5), and §4.6's metadata tap is fed from `event:` lines only so it is *structurally* unable to read content. `GET .../events` proxies Anthropic's JSON verbatim rather than through the SDK's models — the event union is closed there, so typing it would drop the first event kind Anthropic adds, and on a replay proxy fidelity is the feature. Any TLS terminator in front of this route needs its read timeout raised well past nginx's 60s default, or turns get severed during the minutes of silence while a tool runs. | ✅ |
...
- **`plugin/`** — the Brainite Claude Code plugin (Go). Not part of the FastAPI app; it is a *client* of `POST /runs`, and it lives here because it is defined entirely by that contract. `make -C plugin test` builds and tests it. Requires Go (`brew install go`).
