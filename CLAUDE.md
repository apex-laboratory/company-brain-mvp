...
| — Agent builder | User-built agents that reach **their own** tools at runtime over MCP, grounded in the Brain. Runtime is **Anthropic Managed Agents** — the one part of the app that can't ride the `LLM_PROVIDER` factory. Plan: `docs/agent-builder-plan.md`. **Brainite stores pointers — agents, vaults, sessions, schedules — never the content that flows through them**, and that wall is why `agent_credentials` has no token column and `agent_sessions` has no transcript column (migration 0028 says so; §5.5's schema test will enforce it). Phase 0 (streamable-HTTP MCP + bearer auth + the `agent_origin` query-text guard) ✅ and phase 1 (six tables, agent CRUD, connectors, `anthropic_client.py` — the only file that imports the Managed Agents SDK) ✅ are on `main`. Phase 2 (catalog, per-connector OAuth, vaults) is on `feat/agent-builder-credentials`. **Connectors here are data, not code** — a provider is a row in `agent_connectors.yaml` plus env-var client credentials; do *not* copy the compile-time `SourceIntegration` pattern from `app/integrations/`. Remaining: phase 4 sessions + stream proxy, phase 6 schedules, phase 7 wall-enforcement tests. | 🔵 in progress |
